<template>
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 my-10">
        ...
        <div class="md:flex md:h-full md:flex-row-reverse flex flex-col-reverse rounded-3xl overflow-hidden"
            :style="'background:' + project.background_color">
            <!-- Image -->
            <div class="md:shrink-0 md:w-1/2 text-center">
                <nuxt-img preload class="h-full md:float-right m-auto" :src="storage_base_url + project.image"
                    alt="{{project.title}}" loading="lazy" />
            </div>
            <!-- Text -->
            <div class="p-8 pb-2 w-full flex flex-col justify-center">
                <div>
                    <h1 class="slider_title" :class="'text-[' + project.text_color + ']'">{{ project.title }}</h1>
                    <p class="mt-2" :class="'text-[' + project.text_color + ']'">{{
        ...
        <div class="my-20">
            <NewsSlider :slides="projects" title="Вас также может интересовать" source="/projects/"/>
        </div>
    </div>
</template>
<script setup>
...
const { data: project } = await useFetch(api_base_url + 'projects/' + slug, { headers });
